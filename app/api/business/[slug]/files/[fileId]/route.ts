...
export async function DELETE(
  ...
  if (remainingTransactions.length === 0) {
    ...
    await prisma.reportSnapshot.deleteMany({ where: { businessId: business.id } });
    return NextResponse.json({
      success: true,
      report: null,
      message: 'File deleted. No remaining data — dashboard reset to empty state.',
    });
  }
  ...
  try {
    ...
    if (!stdout || !stdout.trim()) {
      throw new Error(stderr || 'Engine produced no output after file deletion.');
    }
