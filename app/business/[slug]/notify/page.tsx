...
const ROLE_BY_FLAG_KEYWORD: Array<[string, string, string]> = [
  ...
  [
    ...
    "Accountant",
    "Verify this payment against a supporting invoice.",
  ],
  [
    "unusually precise",
    "Accountant",
    "Confirm this recipient and payment are legitimate.",
  ],
  [
    "unusual transaction",
    "Accountant",
  ...
  [
    "cash buffer",
    "Founder",
    "Review upcoming payments due and confirm there's enough cash to cover them.",
  ],
];
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      ...
      <span
        ...
        style={{ color: active || done ? "var(--ink)" : "var(--sage)" }}
      >
        {label}
      </span>
    </div>
...
function StepBar({ step }: { step: Step }) {
  return (
    <div className="mb-6 flex items-start justify-center gap-0">
      <StepDot
        active={step === "connect"}
        done={step === "push" || step === "done"}
        label="Connect"
      />
      <div
        className="mx-2 mt-4 h-px w-12 flex-shrink-0"
...
export default function NotifyPage() {
  ...
  async function load() {
    ...
    setError(null);
    try {
      const [report, sheetsStatus] = await Promise.all([
        fetchReport(slug),
        fetchSheetsStatus(),
      ]);
      setData(report);
  ...
  async function handlePush() {
    ...
    setError(null);
    try {
      const pushResult = await pushToSheets(
        data.report,
      ...
      if (e.code === "TOKEN_EXPIRED") {
        ...
        setStatus((s) =>
          s ? { ...s, configured: false, connectedEmail: null } : s,
        );
        setError(
          "Your Google Sheets connection has expired. Please reconnect.",
        );
      } else {
    ...
    <div className="max-w-2xl space-y-5">
      ...
      >
        ...
        <p className="mt-2 text-sm leading-6" style={{ color: "var(--sage)" }}>
          Creates a new Google Sheet in your Drive with an action list sorted by
          priority, each item assigned to the founder or accountant — then
          emails both with the checklist.
        </p>
        ...
        {step === "connect" && (
          <div className="mt-2 space-y-4">
            ...
            >
              ...
              >
                We'll ask Google for permission to create and edit spreadsheets
                in your Drive. The sheet will live in your Google Drive — you
                own it.
              </p>
            </div>
        ...
        {step === "push" && (
          <div className="mt-2 space-y-4">
            ...
            {data && (
              ...
              >
                ...
                <p
                  ...
                  style={{ color: "var(--sage)" }}
                >
                  A new sheet will be created in your Google Drive. The founder
                  and accountant will be emailed with the checklist after the
                  push.
                </p>
              </div>
            )}
            ...
            <button
              ...
              style={{
                background:
                  pushing || sendingEmail ? "var(--sage)" : "var(--ink)",
              }}
            >
              {pushing ? (
                <>
          ...
          <div className="mt-2 space-y-4">
            ...
            >
              ...
              <p className="mt-1 text-xs" style={{ color: "var(--ink)" }}>
                {result.action_items_written} action items and{" "}
                {result.anomaly_rows_written} transaction rows written.
              </p>
              {emailSent && (
                <p className="mt-1 text-xs" style={{ color: "var(--ink)" }}>
                  ✓ Notification email sent to founder
                  {status?.connectedEmail ? "" : ""} and accountant.
                </p>
              )}
            </div>
